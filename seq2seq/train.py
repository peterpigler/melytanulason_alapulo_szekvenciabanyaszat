...
input_seqs, target_seqs, sequences, input_characters, [PAD,EOS,StOS] = dataset.load_sequences('Sequences_alarm_warning_171208.xlsx', 5)
PADfaultclass, y_label, [YEncoder, YEncoder_OH] = dataset.load_faultclasses('Fault_0822.xlsx')
input_seqs, target_seqs, faultclass = dataset.split_sequences_with_labels(input_seqs, target_seqs, sequences, 5, 3, PAD, StOS, EOS, y_label)
...
resval1, resval2, ovres, [output_token_list0, output_token_list1, output_token_list2], tree_seq, tree_conf, states, seqs =train_sequences_crossval(encoder_input_data, decoder_input_data, decoder_target_data, input_token_index, target_token_index, input_seqs, target_seqs, faultclass, EOS, PAD, StOS, max_decoder_seq_length, num_encoder_tokens, num_decoder_tokens,                              40, 256, 256, 100,                             n_splits=1, test_size=0.2, random_state=42,                              modelname=None)
